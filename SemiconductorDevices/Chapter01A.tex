...
\begin{BoxFormula}[PN结的载流子分布]
    PN结在正偏时，少子浓度分别满足
    \begin{Align}[20pt]
        \qquad\fdd{p_n(x)}=p_n(x)-p_{n0}=p_{n0}&\qty[\exp(\frac{eV_a}{\kB T})-1]\exp(\frac{x_n-x}{L_p})\qquad x\geq +x_n\qquad\\
        \qquad\fdd{n_p(x)}=n_p(x)-n_{p0}=n_{p0}&\qty[\exp(\frac{eV_a}{\kB T})-1]\exp(\frac{x_p+x}{L_n})\qquad x\leq -x_p\qquad
    \end{Align}
\end{BoxFormula}
...
\begin{Proof}
    ...
    \xrefpeq{3}的通解为
    \begin{Equation}&[5]
        \fdd{p_n(x)}=p_n(x)-p_{n0}=P_1\e^{x/L_p}+P_2\e^{-x/L_p}\qquad x\geq +x_n
    \end{Equation}
    \xrefpeq{4}的通解为
    \begin{Equation}&[6]
        \fdd{n_p(x)}=n_p(x)-n_{p0}=N_1\e^{x/L_n}+N_2\e^{-x/L_n}\qquad x\leq -x_p
    \end{Equation}
